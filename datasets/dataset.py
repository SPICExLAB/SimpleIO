...
class SeqeuncesDataset(Data.Dataset):
    ...
    def __init__(
        ...
        (
            ...
            self.right_hip_acc,
            self.right_hip_rot,
            self.right_elbow_acc,
            self.right_elbow_rot,
            self.joint_pos,
            self.joint_rot,
            # self.root_vel,
            self.mobileposer_input,
            self.index_map,
            self.seq_idx,
        ) = ([], [], [], [], [], [], [], [],[],[],[],[],[],[],[], 0)
                ...
                if isinstance(conf.data_drive, list):
                    if len(conf.data_drive) == 0:
                        ...
                        for pkl_path in tqdm(
                            ...
                            self.seq_idx += 1
                            import psutil, os; print("memory usage: ", psutil.Process(os.getpid()).memory_info().rss / 1024**2, "MB")
                    else:
                        # Use explicit list

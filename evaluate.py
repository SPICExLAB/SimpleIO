            ...
            if args.exp is not None:
                ...
                if "coordinate" in dataset_conf.keys():
                    ...
                    if dataset_conf["coordinate"] == "glob_coord":
                        ...
                        net_vel = interp_xyz(gt_ts, vel_ts[:,0], inference_state['net_vel']).to(motion_dataset.data['velocity'].dtype)
                ...
                pred_traj = inf_outstate['poses'][0]
                gt_traj = inf_outstate['poses_gt'][1:]
                err = pred_traj - gt_traj
                err_v = err[..., 1].abs()
                err_h = err[..., [0, 2]].norm(dim=-1)
                ate_v = torch.sqrt((err_v ** 2).mean()).item()
                ate_h = torch.sqrt((err_h ** 2).mean()).item()

                final_err = err[-1].norm().item()
                total_dist = (gt_traj[1:] - gt_traj[:-1]).norm(dim=-1).sum().item()
                drift_rate = 100.0 * final_err / total_dist if total_dist > 0 else float('nan')
